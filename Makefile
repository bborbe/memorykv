...
format:
	...
	go run github.com/incu6us/goimports-reviser/v3@$(GOIMPORTS_REVISER_VERSION) -project-name github.com/bborbe/memorykv -format -excludes vendor ./...
...
.PHONY: errcheck
errcheck:
	GOFLAGS="-mod=mod" go run github.com/kisielk/errcheck -ignore '(Close|Write|Fprint)' $(shell go list -mod=mod ./... | grep -v /vendor/ | grep -v k8s/client)
